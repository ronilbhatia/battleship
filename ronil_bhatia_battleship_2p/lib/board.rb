...
class Board
  ...
  def add_ships(player)
    ...
    SHIPS.each do |ship, ship_size|
      puts "Where would you like to place the #{ship}, with size #{ship_size}?"
      ship_coords = gets.chomp.delete(",!?+-/-.() ").split("").map(&:to_i)
      length = (ship_coords[0] - ship_coords[2]).abs + (ship_coords[1] - ship_coords[3]).abs + 1
      while ship_coords.length != 4 || length != ship_size
        puts "Invalid coordinates: please try again"
        print "> "
        ship_coords = gets.chomp.delete(",!?+-/-() ").split("").map(&:to_i)
      end

      if ship_coords[0] == ship_coords[2]
        (ship_coords[1]..ship_coords[3]).each do |y|
          pos = [ship_coords[0], y]
          self[pos] = :s
        end
      else
        (ship_coords[0]..ship_coords[2]).each do |x|
          pos = [x, ship_coords[1]]
          self[pos] = :s
        end
      end
